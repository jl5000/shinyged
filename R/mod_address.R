...
address_server <- function(id, r, addr_rows) {
  moduleServer(id, function(input, output, session) {
    ...
    shiny::observeEvent(input$adr, ignoreNULL = FALSE, ignoreInit = TRUE, {
      ...
      dummy <- update_ged_value(r, addr_rows, addr()$level[1] + 1, paste0("ADR", 1:3), adr, .pkgenv$tags_addr)
    })
    shiny::observeEvent(input$city, ignoreNULL = FALSE, ignoreInit = TRUE, {
      ...
      req(is.null(err), cancelOutput = TRUE)
      dummy <- update_ged_value(r, addr_rows, addr()$level[1] + 1, "CITY", city, .pkgenv$tags_addr)
    })
    shiny::observeEvent(input$state, ignoreNULL = FALSE, ignoreInit = TRUE, {
      ...
      req(is.null(err), cancelOutput = TRUE)
      dummy <- update_ged_value(r, addr_rows, addr()$level[1] + 1, "STAE", state, .pkgenv$tags_addr)
    })
    shiny::observeEvent(input$postcode, ignoreNULL = FALSE, ignoreInit = TRUE, {
      ...
      req(is.null(err), cancelOutput = TRUE)
      dummy <- update_ged_value(r, addr_rows, addr()$level[1] + 1, "POST", postcode, .pkgenv$tags_addr)
    })
    shiny::observeEvent(input$country, ignoreNULL = FALSE, ignoreInit = TRUE, {
      ...
      req(is.null(err), cancelOutput = TRUE)
      dummy <- update_ged_value(r, addr_rows, addr()$level[1] + 1, "CTRY", country, .pkgenv$tags_addr)
    })
    shiny::observeEvent(input$emails, ignoreNULL = FALSE, ignoreInit = TRUE, {
      ...
      req(is.null(err), cancelOutput = TRUE)
      dummy <- update_ged_value(r, addr_rows, addr()$level[1], "EMAIL", emails, .pkgenv$tags_addr)
    })
    shiny::observeEvent(input$phones, ignoreNULL = FALSE, ignoreInit = TRUE, {
      ...
      req(is.null(err), cancelOutput = TRUE)
      dummy <- update_ged_value(r, addr_rows, addr()$level[1], "PHON", phones, .pkgenv$tags_addr)
    })
    shiny::observeEvent(input$faxes, ignoreNULL = FALSE, ignoreInit = TRUE, {
      ...
      req(is.null(err), cancelOutput = TRUE)
      dummy <- update_ged_value(r, addr_rows, addr()$level[1], "FAX", faxes, .pkgenv$tags_addr)
    })
    shiny::observeEvent(input$websites, ignoreNULL = FALSE, ignoreInit = TRUE, {
      ...
      req(is.null(err), cancelOutput = TRUE)
      dummy <- update_ged_value(r, addr_rows, addr()$level[1], "WWW", websites, .pkgenv$tags_addr)
    })
